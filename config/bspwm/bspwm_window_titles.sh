...
xev -root -1 -event property | while read -r evt mon _; do
    ...
    for window_id in $winids_on_desktop; do
        # get window props
        window_props=$(xprop -notype -id $window_id WM_NAME _NET_WM_NAME WM_CLASS 2>/dev/null)
        # If xprop failed, don't show the window
        [[ $? != 0 ]] && continue
        window_name=$(echo "$window_props" | grep WM_NAME -m 1 | sed 's/^[A-Z_]* = "\(.*\)"$/\1/')
        ...
        window_class=$(echo "$window_props" | grep WM_CLASS -m 1 | sed 's/^[A-Z_]* = "\([^"]*\)".*$/\1/' )
