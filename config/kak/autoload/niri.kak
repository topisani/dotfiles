...
declare-option -hidden str-to-str-map niri_focus_pid

# these hooks maintain a mapping of client names to niri window IDs.
hook -group "niri-focus" global ClientCreate .* %{
    hook -group "niri-%val{client}" global ClientRenamed "%val{client}:.*" %{
        evaluate-commands %sh{
            IFS=:
            set -- $kak_hook_param
            old=$1
            new=$2
            unset IFS
            printf 'set-option -remove global niri_focus_pid "%s="\n' "$old"
            id="$(niri msg -j focused-window | sed -n 's/.*"id":[[:space:]]*\([0-9][0-9]*\).*/\1/p')"
            printf 'set-option -add global niri_focus_pid "%s=%s"\n' "$new" "$id"
        }
        rmhooks global "niri-%val{client}"
    }
    hook -group "niri-%val{client}" global ClientClose %val{client} %{
        set-option -remove global niri_focus_pid "%val{client}="
        rmhooks global "niri-%val{client}"
    }
    evaluate-commands %sh{
        id="$(niri msg -j focused-window | sed -n 's/.*"id":[[:space:]]*\([0-9][0-9]*\).*/\1/p')"
...
define-command -params ..1 niri-focus -docstring '
niri-focus-client [<kakoune_client>]: focus a given client''s window.
...
%{
    nop %sh{
        ...
        eval set -- "$kak_quoted_opt_niri_focus_pid"
        split_and_check () {
            IFS==
            set -- $1
            client=$1
...
complete-command -menu niri-focus client

alias global wayland-focus niri-focus
alias global focus niri-focus
