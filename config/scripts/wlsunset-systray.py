...
# ]
# ///
"""System tray control for wlsunset color temperature."""
...
def icon_svg(color):
    return f"""<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="2 2 18 18" fill="{color}">
...
ICON_OFF = icon_from_svg(icon_svg("#777777"))
ICON_ON = icon_from_svg(icon_svg("#FFFFFF"))
...
def update_icon():
    if enabled:
        icon.set_icon(pixmap=ICON_ON)
        icon.set_tooltip(f"Color Temperature Control (Enabled - {target_temp}K)")
    else:
        icon.set_icon(pixmap=ICON_OFF)
        icon.set_tooltip("Color Temperature Control (Disabled)")
...
def update_menu():
    ...
    temp_children = [
        MenuItem(
            label,
            toggle_type="radio",
            toggle_state=1 if temp == target_temp else 0,
            on_click=lambda t=temp: change_temp(t),
        )
    ...
    icon.menu = [
        ...
        Separator(),
        MenuItem("Temperature", children=temp_children),
        Separator(),
        MenuItem("Quit", on_click=lambda: (reset_temperature() if enabled else None, icon.stop())),
    ]
